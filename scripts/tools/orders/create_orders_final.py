"""
DEPRECATED: Do not use for Bybit trading. This script sets consensus.trade_id to
orders.id (not trades.id), which breaks Consensus vs Trading tab consistency.
Use activate_consensus / Place Trade in GUI or scripts/tools/repair_consensus_trade_links.py.
"""
import sqlite3
import sys
import os
...
def create_orders_manually():
    ...
    for record in records:
        ...
        try:
            ...
            # Create parent order with correct schema
            cursor.execute("""
                INSERT INTO orders (
            ...
            """, (
                ...
                float(5 if signal == "LONG" else -5),  # quantity
                float(entry_limit_price) if entry_limit_price else None,  # limit_price
                None,  # stop_price
                "QUEUED",  # status
                "",  # account_type
                now,  # created_at
...
def main():
    ...
    # Show new orders
    cursor.execute("""
        SELECT id, ticker, status, order_role, created_at
        FROM orders 
        WHERE status = 'QUEUED' 
